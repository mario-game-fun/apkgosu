...
if (ENVIRONMENT_IS_NODE) {
    ...
    ;
    readAsync = function() {
        var _ref = _asyncToGenerator(_regeneratorRuntime().mark(function _callee(filename) {
            var binary, ret, _args = arguments;
...
function _getWasmBinary() {
    _getWasmBinary = _asyncToGenerator(_regeneratorRuntime().mark(function _callee4(binaryFile) {
        var response;
...
function _instantiateArrayBuffer() {
    _instantiateArrayBuffer = _asyncToGenerator(_regeneratorRuntime().mark(function _callee5(binaryFile, imports) {
        var binary, instance;
...
function _instantiateAsync() {
    _instantiateAsync = _asyncToGenerator(_regeneratorRuntime().mark(function _callee6(binary, binaryFile, imports) {
...
function _createWasm() {
    _createWasm = _asyncToGenerator(_regeneratorRuntime().mark(function _callee7() {
...
var tempDouble;
var tempI64;
var ExitStatus = _createClass(function ExitStatus(status) {
    "use strict";
...
var PokiSdk = {
    ...
    _urlParameters: null,
    _messagesHashTable: {},
    _commercialBreakCallback: function _commercialBreakCallback() {
        getWasmTableEntry(PokiSdk._callback)()
    },
    _rewardedBreakCallback: function _rewardedBreakCallback(success) {
        var msg = success ? PokiSdk.REWARDED_BREAK_SUCCESS : PokiSdk.REWARDED_BREAK_ERROR;
        getWasmTableEntry(PokiSdk._callback)(msg)
    },
    _rewardedBreakStartedCallback: function _rewardedBreakStartedCallback() {
        var msg = PokiSdk.REWARDED_BREAK_START;
        getWasmTableEntry(PokiSdk._callback)(msg)
    },
    _shareableURLCallback: function _shareableURLCallback(url) {
        var _url = stringToNewUTF8(url);
        getWasmTableEntry(PokiSdk._urlCallback)(_url, lengthBytesUTF8(url));
        Module._free(_url)
    }
};
...
function _PokiSdkJs_InternalCaptureError(formatted_string) {
...
function _PokiSdkJs_Measure(category, what, action) {
    PokiSDK.measure(UTF8ToString(category), UTF8ToString(what), UTF8ToString(action))
}
function _PokiSdkJs_MovePill(topPercent, topPx) {
    // PokiSDK.movePill(topPercent, topPx)
}
function _PokiSdkJs_RewardedBreak(size, callback) {
    PokiSdk._callback = callback;
    var options = {
...
var IDBFS = {
    ...
    mount: function mount(_mount) {
        var _mount$opts;
        var mnt = MEMFS.mount(_mount);
        if (_mount !== null && _mount !== void 0 && (_mount$opts = _mount.opts) !== null && _mount$opts !== void 0 && _mount$opts.autoPersist) {
            ...
            mnt.node_ops.mkdir = function() {
                return IDBFS.queuePersist(mnt.mount),
                memfs_node_ops.mkdir.apply(memfs_node_ops, arguments)
            }
            ;
            mnt.node_ops.rmdir = function() {
                return IDBFS.queuePersist(mnt.mount),
                memfs_node_ops.rmdir.apply(memfs_node_ops, arguments)
            }
            ;
            mnt.node_ops.symlink = function() {
                return IDBFS.queuePersist(mnt.mount),
                memfs_node_ops.symlink.apply(memfs_node_ops, arguments)
            }
            ;
            mnt.node_ops.unlink = function() {
                return IDBFS.queuePersist(mnt.mount),
                memfs_node_ops.unlink.apply(memfs_node_ops, arguments)
            }
            ;
            mnt.node_ops.rename = function() {
                return IDBFS.queuePersist(mnt.mount),
                memfs_node_ops.rename.apply(memfs_node_ops, arguments)
            }
        }
        return mnt
    },
...
var FS = {
    ...
    syncFSRequests: 0,
    readFiles: {},
    ErrnoError: _createClass(function ErrnoError(errno) {
        "use strict";
    ...
    mkdirTree(path, mode) {
        var dirs = path.split("/");
        var d = "";
        var _iterator = _createForOfIteratorHelper(dirs), _step;
        try {
    ...
    quit() {
        ...
        var _iterator2 = _createForOfIteratorHelper(FS.streams), _step2;
        try {
...
var ___syscall__newselect = function ___syscall__newselect(nfds, readfds, writefds, exceptfds, timeout) {
    try {
        var total = 0;
...
var SOCKFS = {
    ...
    emit(event, param) {
        var _SOCKFS$callbacks$eve, _SOCKFS$callbacks;
    ...
    websocket_sock_ops: {
        ...
        close(sock) {
            ...
            for (var _i2 = 0, _Object$values = Object.values(sock.peers); _i2 < _Object$values.length; _i2++) {
                var peer = _Object$values[_i2];
                try {
                    peer.socket.close()
                } catch (e) {}
                SOCKFS.websocket_sock_ops.removePeer(sock, peer)
...
var zeroMemory = function zeroMemory(ptr, size) {
...
var getSocketAddress = function getSocketAddress(addrp, addrlen) {
...
var syscallGetVarargI = function syscallGetVarargI() {
...
var checkWasiClock = function checkWasiClock(clock_id) {
...
function _clock_time_get(clk_id, ignored_precision_low, ignored_precision_high, ptime) {
    ...
    tempI64 = [nsec >>> 0, (tempDouble = nsec,
    +Math.abs(tempDouble) >= 1 ? tempDouble > 0 ? +Math.floor(tempDouble / 4294967296) >>> 0 : ~~+Math.ceil((tempDouble - +(~~tempDouble >>> 0)) / 4294967296) >>> 0 : 0)],
...
function _dmDeviceJSOpen(bufferCount) {
    ...
    if (window.AudioContext || window.webkitAudioContext) {
        ...
        device = {
            ...
            _queue: function _queue(samples, frame_count) {
...
var JSEvents = {
    ...
    deferCall(targetFunction, precedence, argsList) {
        ...
        var _iterator3 = _createForOfIteratorHelper(JSEvents.deferredCalls), _step3;
        try {
    ...
    runDeferredCalls() {
        ...
        JSEvents.deferredCalls = [];
        var _iterator4 = _createForOfIteratorHelper(deferredCalls), _step4;
        try {
    ...
    if (mode == 0) {
        ...
        if (typeof MainLoop.setImmediate == "undefined") {
            if (typeof setImmediate == "undefined") {
                ...
                MainLoop.setImmediate = function(func) {
...
var MainLoop = {
    ...
    runIter(func) {
        if (ABORT)
            return;
        var _iterator5 = _createForOfIteratorHelper(MainLoop.preMainLoop), _step5;
        try {
            for (_iterator5.s(); !(_step5 = _iterator5.n()).done; ) {
                var pre = _step5.value;
                if (pre() === false) {
                    return
                }
            }
        ...
        var _iterator6 = _createForOfIteratorHelper(MainLoop.postMainLoop), _step6;
        try {
...
var webgl_enable_EXT_polygon_offset_clamp = function webgl_enable_EXT_polygon_offset_clamp(ctx) {
    return !!(ctx.extPolygonOffsetClamp = ctx.getExtension("EXT_polygon_offset_clamp"))
};
var webgl_enable_EXT_clip_control = function webgl_enable_EXT_clip_control(ctx) {
    return !!(ctx.extClipControl = ctx.getExtension("EXT_clip_control"))
};
var webgl_enable_WEBGL_polygon_mode = function webgl_enable_WEBGL_polygon_mode(ctx) {
...
var GL = {
    ...
    deleteContext: function deleteContext(contextHandle) {
        ...
        if ((_GL$contexts$contextH = GL.contexts[contextHandle]) !== null && _GL$contexts$contextH !== void 0 && _GL$contexts$contextH.GLctx.canvas) {
...
var _glClipControlEXT = function _glClipControlEXT(origin, depth) {
...
var _glGetString = function _glGetString(name_) {
    ...
    if (!ret) {
        ...
        case 7938:
            ...
            var glVersion = "OpenGL ES 2.0 (".concat(webGLVersion, ")");
            if (GL.currentContext.version >= 2)
                glVersion = "OpenGL ES 3.0 (".concat(webGLVersion, ")");
...
var _glPolygonModeWEBGL = function _glPolygonModeWEBGL(face, mode) {
...
var _glPolygonOffsetClampEXT = function _glPolygonOffsetClampEXT(factor, units, clamp) {
...
var _emscripten_set_main_loop_arg = function _emscripten_set_main_loop_arg(func, arg, fps, simulateInfiniteLoop) {
    var iterFunc = function iterFunc() {
...
var GLFW = {
    ...
    DOMToGLFWKeyCode: function DOMToGLFWKeyCode(keycode, code) {
        console.log("DOMToGLFWKeyCode keycode = " + keycode + " code = " + code);
        switch (keycode) {
        case 8:
            return 295;
        case 9:
    ...
    onWindowClose: function onWindowClose(event) {
        GLFW.params[131073] = false
    },
...
function _glfwSetWindowBackgroundColor(color) {}
...
var dmRenderer = {
    renderContext: null,
    renderCallback: null,
    CONTEXT_LOST_EVENT: 0,
    CONTEXT_RESTORED_EVENT: 1,
    rendererContextEvent: function rendererContextEvent(event_type) {
        if (dmRenderer.renderCallback) {
            getWasmTableEntry(dmRenderer.renderCallback)(dmRenderer.renderContext, event_type)
        }
    }
};
function _setupCallbackJS(context, callback) {
...
for (var _i3 = 0; _i3 < 32; ++_i3)
    tempFixedLengthArray.push(new Array(_i3));
...
var wasmImports = {
    ki: _PokiSdkJs_AddParameterForURL,
    ji: _PokiSdkJs_CaptureError,
    ii: _PokiSdkJs_CommercialBreak,
    hi: _PokiSdkJs_GameplayStart,
    gi: _PokiSdkJs_GameplayStop,
    fi: _PokiSdkJs_GetURLParam,
    ei: _PokiSdkJs_InternalCaptureError,
    di: _PokiSdkJs_IsAdBlocked,
    ci: _PokiSdkJs_Measure,
    bi: _PokiSdkJs_MovePill,
    ai: _PokiSdkJs_RewardedBreak,
    $h: _PokiSdkJs_SetDebug,
    _h: _PokiSdkJs_ShareableURL,
    b: ___assert_fail,
    ...
    N: _glfwGetDefaultFramebuffer,
    da: _glfwGetDisplayScaleFactor,
...
var _wasm_call_ctors = function ___wasm_call_ctors() {
    return (_wasm_call_ctors = wasmExports["li"])()
};
var _dmExportedSymbols = Module["_dmExportedSymbols"] = function() {
    return (_dmExportedSymbols = Module["_dmExportedSymbols"] = wasmExports["mi"])()
}
;
var _main = Module["_main"] = function(a0, a1) {
    return (_main = Module["_main"] = wasmExports["ni"])(a0, a1)
}
;
var _malloc = Module["_malloc"] = function(a0) {
    return (_malloc = Module["_malloc"] = wasmExports["oi"])(a0)
}
;
var _free = Module["_free"] = function(a0) {
    return (_free = Module["_free"] = wasmExports["pi"])(a0)
}
;
var _dmEngineSetUpdateEnabled = Module["_dmEngineSetUpdateEnabled"] = function(a0) {
    return (_dmEngineSetUpdateEnabled = Module["_dmEngineSetUpdateEnabled"] = wasmExports["qi"])(a0)
}
;
var _dmEngineSetRenderEnabled = Module["_dmEngineSetRenderEnabled"] = function(a0) {
    return (_dmEngineSetRenderEnabled = Module["_dmEngineSetRenderEnabled"] = wasmExports["ri"])(a0)
}
;
var _htonl2 = function _htonl(a0) {
    return (_htonl2 = wasmExports["ti"])(a0)
};
var _dmScript_Html5ReportOperationSuccess = Module["_dmScript_Html5ReportOperationSuccess"] = function(a0) {
    return (_dmScript_Html5ReportOperationSuccess = Module["_dmScript_Html5ReportOperationSuccess"] = wasmExports["ui"])(a0)
}
;
var _dmScript_RunInteractionCallback = Module["_dmScript_RunInteractionCallback"] = function() {
    return (_dmScript_RunInteractionCallback = Module["_dmScript_RunInteractionCallback"] = wasmExports["vi"])()
}
;
var _htons2 = function _htons(a0) {
    return (_htons2 = wasmExports["wi"])(a0)
};
var _ntohs2 = function _ntohs(a0) {
    return (_ntohs2 = wasmExports["xi"])(a0)
};
var _JSWriteDump = Module["_JSWriteDump"] = function(a0) {
    return (_JSWriteDump = Module["_JSWriteDump"] = wasmExports["yi"])(a0)
}
;
var _setThrew2 = function _setThrew(a0, a1) {
    return (_setThrew2 = wasmExports["zi"])(a0, a1)
};
var _emscripten_tempret_set = function __emscripten_tempret_set(a0) {
    return (_emscripten_tempret_set = wasmExports["Ai"])(a0)
};
var _emscripten_stack_restore = function __emscripten_stack_restore(a0) {
    return (_emscripten_stack_restore = wasmExports["Bi"])(a0)
};
var _emscripten_stack_alloc = function __emscripten_stack_alloc(a0) {
    return (_emscripten_stack_alloc = wasmExports["Ci"])(a0)
};
var _emscripten_stack_get_current2 = function _emscripten_stack_get_current() {
    return (_emscripten_stack_get_current2 = wasmExports["Di"])()
};
