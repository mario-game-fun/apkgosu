...
window.DOMHandler = class {
    ...
    Attach() {}
    PostToRuntime(t, i, e, s) {
        this._iRuntime.PostToRuntimeComponent(this._componentId, t, i, e, s)
    }
    PostToRuntimeAsync(t, i, e, s) {
        return this._iRuntime.PostToRuntimeComponentAsync(this._componentId, t, i, e, s)
    }
    _PostToRuntimeMaybeSync(t, i, e) {
        this._iRuntime.UsesWorker() ? this.PostToRuntime(t, i, e) : this._iRuntime._OnLocalRuntimeMessageFromDOM({
            "type": "event",
    ...
    AddRuntimeMessageHandlers(t) {
        for (const [i,e] of t)
            this.AddRuntimeMessageHandler(i, e)
    }
    GetRuntimeInterface() {
...
window.RateLimiter = class {
    ...
    Call() {
        if (-1 !== this._timerId)
            return;
        const t = Date.now()
          , i = t - this._lastCallTime
          , e = this._interval;
        i >= e && this._canRunImmediate ? (this._lastCallTime = t,
        this._RunCallback()) : this._timerId = self.setTimeout(this._timerCallFunc, Math.max(e - i, 4))
    }
    _RunCallback() {
        this._ignoreReset = !0,
        this._callback(),
...
"use strict";
{
    ...
    window.DOMElementHandler = class extends self.DOMHandler {
        ...
        AddDOMElementMessageHandlers(e) {
            for (const [t,n] of e)
                this.AddDOMElementMessageHandler(t, n)
        }
        _OnCreate(t) {
            const n = t["elementId"]
              , s = this.CreateElement(n, t)
              , i = new e(s);
            this._elementMap.set(n, i),
            s.style.boxSizing = "border-box",
            s.style.display = "none",
            i.SetVisibleFlag(t["isVisible"]);
            const l = this._GetFocusElement(s);
            l.addEventListener("focus", e => this._OnFocus(n)),
            l.addEventListener("blur", e => this._OnBlur(n));
            const a = t["htmlIndex"];
            if (i.SetWantHTMLIndex(a),
            i.SetHTMLZIndex(t["htmlZIndex"]),
            this._autoAttach) {
                const e = this.GetRuntimeInterface().GetAvailableHTMLIndex(a);
                i.SetActualHTMLIndex(e);
                this.GetRuntimeInterface().GetHTMLWrapElement(e).appendChild(s)
            }
        }
        CreateElement(e, t) {
        ...
        _OnSetVisible(e) {
            ...
            const t = this._elementMap.get(e["elementId"])
              , n = t.GetElement();
            t.HadFirstUpdate() ? n.style.display = e["isVisible"] ? "" : "none" : t.SetVisibleFlag(e["isVisible"])
        }
        _OnUpdatePosition(e) {
            if (!this._autoAttach)
                return;
            ...
            const i = e["fontSize"];
            null !== i && (n.style.fontSize = i + "em");
            const l = e["htmlIndex"];
            t.SetWantHTMLIndex(l);
            const a = s.GetAvailableHTMLIndex(l);
            if (a !== t.GetActualHTMLIndex()) {
                n.remove();
                s.GetHTMLWrapElement(a).appendChild(n),
                t.SetActualHTMLIndex(a),
                s._UpdateHTMLElementsZOrder()
            }
            const d = e["htmlZIndex"];
            d !== t.GetHTMLZIndex() && (t.SetHTMLZIndex(d),
            s._UpdateHTMLElementsZOrder()),
            t.HadFirstUpdate() || (t.SetHadFirstUpdate(),
            t.GetVisibleFlag() && (n.style.display = ""))
        }
            ...
            if (this._autoAttach)
                for (const e of this._elementMap.values()) {
                    ...
                    if (-1 !== t && -1 !== n && t !== n) {
                        ...
                        n.remove();
                        this.GetRuntimeInterface().GetHTMLWrapElement(t).appendChild(n),
                        e.SetActualHTMLIndex(t)
                    }
                }
        }
        ...
        _OnSetCssStyle(e) {
            const t = this.GetElementById(e["elementId"])
              , n = e["prop"]
              , s = e["val"];
            n.startsWith("--") ? t.style.setProperty(n, s) : t.style[n] = s
        }
        _OnSetAttribute(e) {
...
"use strict";
{
    const e = /(iphone|ipod|ipad|macos|macintosh|mac os x)/i.test(navigator.userAgent)
      ...
      , s = /safari/i.test(navigator.userAgent) && !/(chrome|chromium|edg\/|OPR\/)/i.test(navigator.userAgent);
    let n = 0;
    function r(e) {
        const t = document.createElement("script");
        return t.async = !1,
        t.type = "module",
        e.isStringSrc ? new Promise(s => {
            const r = "c3_resolve_" + n;
            ++n,
            self[r] = s,
            t.textContent = e.str + `\n\nself["${r}"]();`,
    ...
    async function i() {
        ...
        try {
            ...
            const t = new Blob(['\n\tself.addEventListener("message", () =>\n\t{\n\t\ttry {\n\t\t\tconst offscreenCanvas = new OffscreenCanvas(32, 32);\n\t\t\tconst gl = offscreenCanvas.getContext("webgl");\n\t\t\tself.postMessage(!!gl);\n\t\t}\n\t\tcatch (err)\n\t\t{\n\t\t\tconsole.warn("Feature detection worker error:", err);\n\t\t\tself.postMessage(false);\n\t\t}\n\t});'],{
              ...
              , s = new Worker(URL.createObjectURL(t),{
                get type() {
                    e = !0
                }
            })
            ...
            return console.warn("Error feature detecting worker mode: ", e),
            !1
        }
    }
    let a = new Audio;
    ...
    let w = !1;
    window.RuntimeInterface = class n {
        #e = !0;
        #t = null;
        #s = "";
        #n = "";
        ...
        #C = "";
        constructor(e) {
            if (this.#e = e.useWorker,
            this.#n = e.scriptFolder,
            this.#C = e.exportType,
            "playable-ad-single-file" !== this.#C && "playable-ad-zip" !== this.#C && "instant-games" !== this.#C || (this.#e = !1),
            (s || "macos-wkwebview" === this.#C) && (this.#e = !1),
            "cordova" === this.#C && this.#e && t) {
                const e = /Chrome\/(\d+)/i.exec(navigator.userAgent);
                e && parseInt(e[1], 10) >= 90 || (this.#e = !1)
            }
            this.IsAnyWebView2Wrapper() ? self["chrome"]["webview"].addEventListener("message", e => this.#I(e.data, e["additionalObjects"])) : "macos-wkwebview" === this.#C ? self["C3WrapperOnMessage"] = e => this.#I(JSON.parse(e)) : "linux-cef" === this.#C && self["c3_linux_cef_set_message_callback"](e => this.#I(JSON.parse(e))),
            this._localFileBlobs = null,
            this._localFileStrings = null,
            "html5" !== this.#C || window.isSecureContext || console.warn("[Construct] Warning: the browser indicates this is not a secure context. Some features may be unavailable. Use secure (HTTPS) hosting to ensure all features are available."),
            this.AddRuntimeComponentMessageHandler("canvas", "update-size", e => this.#S(e)),
            this.AddRuntimeComponentMessageHandler("canvas", "set-html-layer-count", e => this["_OnSetHTMLLayerCount"](e)),
            this.AddRuntimeComponentMessageHandler("canvas", "cleanup-html-layers", () => this.#L()),
            this.AddRuntimeComponentMessageHandler("canvas", "update-html-layer-dom-state", e => this.#P(e["layersDomState"])),
            this.AddRuntimeComponentMessageHandler("runtime", "create-job-worker", e => this.#O(e)),
            this.AddRuntimeComponentMessageHandler("runtime", "send-wrapper-extension-message", e => this.#W(e)),
            this.AddRuntimeComponentMessageHandler("runtime", "send-raw-wrapper-extension-message", e => this.#H(e)),
            "cordova" === this.#C ? document.addEventListener("deviceready", () => this.#x(e)) : this.#x(e),
            this._skipAndroidVirtualKeyboardDetection = 0
        }
        Release() {
            this._CancelAnimationFrame(),
            this.#t && (this.#t.onmessage = null,
            this.#t = null),
            this.#r && (this.#r.terminate(),
            this.#r = null),
            this.#i && (this.#i.Release(),
            this.#i = null);
            for (const {canvas: e, htmlWrap: t} of this.#d)
                e.remove(),
                t.remove();
            this.#d.length = 0
        }
        GetMainCanvas() {
            return this.#d[0].canvas
        }
        GetAvailableHTMLIndex(e) {
            return Math.min(e, this.#d.length - 1)
        }
        GetHTMLWrapElement(e) {
            if (e < 0 || e >= this.#d.length)
                throw new RangeError("invalid canvas layer");
            return this.#d[e].htmlWrap
        }
        ...
        IsiOSCordova() {
            return e && "cordova" === this.#C
        }
        IsiOSWebView() {
            const t = navigator.userAgent;
            return e && u(this.#C) || navigator["standalone"] || /crios\/|fxios\/|edgios\//i.test(t)
        }
        IsAndroid() {
            return t
        }
        ...
        IsWindowsWebView2() {
            return "windows-webview2" === this.#C || !!("preview" === this.#C && window["chrome"] && window["chrome"]["webview"] && window["chrome"]["webview"]["postMessage"])
        }
        IsAnyWebView2Wrapper() {
            return this.IsWindowsWebView2() || "xbox-uwp-webview2" === this.#C
        }
        ...
        *_CanDoAndroidVirtualKeyboardDetection() {
            ...
            this._skipAndroidVirtualKeyboardDetection > 0 && this._skipAndroidVirtualKeyboardDetection--
        }
        async #x(e) {
            if (this.#e) {
                await i() || (this.#e = !1)
            }
            if ("macos-wkwebview" === this.#C && this._SendWrapperMessage({
                "type": "ready"
            }),
            this.IsAnyWebView2Wrapper() || "macos-wkwebview" === this.#C || "linux-cef" === this.#C) {
                this.#k();
                const e = await this.#E();
                this.#T = e["registeredComponentIds"]
            }
            if ("playable-ad-single-file" === this.#C && (this._localFileBlobs = self["c3_base64files"],
            this._localFileStrings = {},
            await this.#_()),
            e.runtimeBaseUrl)
                this.#s = e.runtimeBaseUrl;
            else {
                const e = location.origin;
                this.#s = ("null" === e ? "file:///" : e) + location.pathname;
                const t = this.#s.lastIndexOf("/");
                -1 !== t && (this.#s = this.#s.substr(0, t + 1))
            }
            const t = new MessageChannel;
            if (this.#t = t.port1,
            this.#t.onmessage = e => this["_OnMessageFromRuntime"](e.data),
            window["c3_addPortMessageHandler"] && window["c3_addPortMessageHandler"](e => this.#A(e)),
            this.#y = new self.JobSchedulerDOM(this),
            await this.#y.Init(),
            ...
            this.#e ? await this.#F(e, t.port2) : await this.#D(e, t.port2)
        }
        #U(e) {
            return {
                "runtimeBaseUrl": this.#s,
                ...
                "isFullscreen": n.IsDocumentFullscreen(),
                ...
                "ife": !!self.ife,
                "jobScheduler": this.#y.GetPortData(),
                "supportedAudioFormats": o,
                ...
                "wrapperComponentIds": this.#T,
                "isFBInstantAvailable": void 0 !== self["FBInstant"]
            }
        }
        async #F(e, t) {
            const s = e.workerMainUrl;
            "preview" === this.#C ? (this.#r = new Worker("previewworker.js",{
                type: "module",
                name: "Runtime"
            }),
            await new Promise( (e, t) => {
                const n = s => {
                    this.#r.removeEventListener("message", n),
                    s.data && "ok" === s.data["type"] ? e() : t()
                }
                ;
                this.#r.addEventListener("message", n),
                this.#r.postMessage({
                    "type": "construct-worker-init",
                    "import": new URL(s,this.#s).toString()
                })
            }
            )) : this.#r = await this.CreateWorker(s, {
                type: "module",
                name: "Runtime"
            });
            const n = document.createElement("canvas");
            n.style.display = "none";
            const r = n["transferControlToOffscreen"]();
            document.body.appendChild(n);
            const i = document.createElement("div");
            i.className = "c3htmlwrap",
            i.setAttribute("interactive", ""),
            ...
            this.#r.postMessage(Object.assign(this.#U(e), {
                "type": "init-runtime",
                "isInWorker": !0,
                "messagePort": t,
                "canvas": r,
            ...
            }), [t, r, ...this.#y.GetPortTransferables()]),
            this.#a = l.map(e => new e(this)),
            this.#B(),
            this.#o._AddDefaultCanvasEventHandlers(n),
            this.#o._AddDefaultHTMLWrapEventHandlers(i),
            this.#o._EnableWindowResizeEvent(),
            self["c3_callFunction"] = (e, t) => this.#o._InvokeFunctionFromJS(e, t),
            "preview" === this.#C && (self["goToLastErrorScript"] = () => this.PostToRuntimeComponent("runtime", "go-to-last-error-script"))
        }
        async #D(e, t) {
            ...
            n.className = "c3htmlwrap",
            n.setAttribute("interactive", ""),
            document.body.appendChild(n),
            this.#d.push({
                canvas: s,
                htmlWrap: n,
            ...
            this.#a = l.map(e => new e(this)),
            this.#B(),
            this.#o._AddDefaultCanvasEventHandlers(s),
            this.#o._AddDefaultHTMLWrapEventHandlers(n);
            const i = await Promise.all(e.runtimeScriptList.map(e => this.#V(e)));
            await Promise.all(i.map(e => r(e)));
            const a = e.projectMainScriptPath
              , o = e.javaScriptInEventsPath
              , d = e.typeScriptInEventsPath;
            if (a)
                try {
                    if (await r(a),
                    "preview" === this.#C && !globalThis.C3_ProjectMainScriptOK)
                        throw new Error("main script did not run to completion")
                } catch (e) {
                    this.#G(),
                    console.error("Error loading project main script: ", e),
                    alert(`Failed to load the project main script (${a}). Check all your JavaScript code has valid syntax, all imports are written correctly, and that an exception was not thrown running the script. Press F12 and check the console for error details.`)
                }
            if (o)
                try {
                    if (await r(o),
                    "preview" === this.#C && !globalThis.C3.JavaScriptInEvents)
                        throw new Error("JavaScript in events did not run to completion")
                } catch (e) {
                    this.#G(),
                    console.error("Error loading JavaScript in events: ", e),
            ...
            if (d)
                try {
                    if (await r(d),
                    "preview" === this.#C && !globalThis.C3.TypeScriptInEvents)
                        throw new Error("TypeScript in events did not run to completion")
                } catch (e) {
                    this.#G(),
                    console.error("Error loading TypeScript in events: ", e),
            ...
            const c = Object.assign(this.#U(e), {
                "isInWorker": !1,
                "messagePort": t,
                "canvas": s,
            ...
            this.#o._EnableWindowResizeEvent(),
            this.#j(),
            this.#i = self["C3_CreateRuntime"](c),
            await self["C3_InitRuntime"](this.#i, c)
        }
        async CreateWorker(e, t) {
            if (e.startsWith("blob:"))
                return new Worker(e,t);
            if ("playable-ad-single-file" === this.#C) {
                const s = this._localFileBlobs[e];
                if (!s)
            ...
            if (location.origin !== s.origin) {
                ...
                return new Worker(URL.createObjectURL(n),t)
            }
        ...
        GetCssDisplayMode() {
            ...
            return new Set(["cordova", "macos-wkwebview", "linux-cef"]).has(e) ? "standalone" : window.matchMedia("(display-mode: fullscreen)").matches ? "fullscreen" : window.matchMedia("(display-mode: standalone)").matches ? "standalone" : window.matchMedia("(display-mode: minimal-ui)").matches ? "minimal-ui" : navigator["standalone"] ? "standalone" : "browser"
        }
        #j() {
            this.#G()
        }
        ...
        async #O(e) {
            const t = await this.#y._CreateJobWorker();
            return {
                "outputPort": t,
                "transferables": [t]
            }
        ...
        #S(e) {
            ...
            const t = e["styleWidth"] + "px"
              ...
              , n = e["marginLeft"] + "px"
              , r = e["marginTop"] + "px";
            for (const {canvas: e, htmlWrap: i} of this.#d)
                e.style.width = t,
                e.style.height = s,
        ...
        "_OnSetHTMLLayerCount"(e) {
            ...
            if (t < this.#d.length)
                for (; this.#d.length > t; ) {
                    const {canvas: e, htmlWrap: t} = this.#d.pop();
                    t.remove(),
                    this.#e && !n ? this.#c.push(e) : e.remove()
                }
            else if (t > this.#d.length)
                for (let e = 0, s = t - this.#d.length; e < s; ++e) {
                    const e = document.createElement("canvas");
                    if (e.classList.add("c3overlay"),
                    this.#e) {
                    ...
                    t.classList.add("c3htmlwrap", "c3overlay"),
                    t.setAttribute("interactive", ""),
                    document.body.appendChild(t),
                    e.style.width = r,
                    e.style.height = i,
                    e.style.marginLeft = a,
                    e.style.marginTop = o,
                    t.style.width = r,
                    t.style.height = i,
                    t.style.marginLeft = a,
                    t.style.marginTop = o,
                    this.#o._AddDefaultCanvasEventHandlers(e),
                    this.#o._AddDefaultHTMLWrapEventHandlers(t),
                    this.#d.push({
                        canvas: e,
                        htmlWrap: t,
                        lastHtmlLayerDomState: {
            ...
            for (const e of this.#a)
                e instanceof window.DOMElementHandler && e._OnHTMLLayersChanged();
        ...
        #P(e) {
            for (let t = 0, s = Math.min(this.#d.length, e.length); t < s; ++t) {
                const {htmlWrap: s, lastHtmlLayerDomState: n} = this.#d[t]
                  , r = e[t]
                  , i = r["isVisible"]
                  , a = r["opacity"]
                  , o = r["isInteractive"];
                i !== n.isVisible && (s.style.display = i ? "" : "none",
                n.isVisible = i),
                a !== n.opacity && (s.style.opacity = 1 === a ? "" : String(a),
                n.opacity = a),
                o !== n.isInteractive && (s.style.pointerEvents = o ? "" : "none",
                o ? s.setAttribute("interactive", "") : s.removeAttribute("interactive"),
                n.isInteractive = o)
            }
        }
        #L() {
        ...
        _UpdateHTMLElementsZOrder() {
            this.#p || (this.#p = !0,
            this._AddRAFCallback(this.#h))
        }
        #m() {
            this._RemoveRAFCallback(this.#h),
            this.#p = !1;
            let e = [];
            for (const t of this.#a)
                if (t instanceof window.DOMElementHandler) {
                    const s = t._GetAllElementStatesForZOrderUpdate();
                    s && e.push(...s)
                }
            e.sort( (e, t) => {
                const s = e.GetActualHTMLIndex()
                  , n = t.GetActualHTMLIndex();
                if (s !== n)
                    return s - n;
                return e.GetHTMLZIndex() - t.GetHTMLZIndex()
            }
            );
            let t = 0
              , s = 0
            ...
            for (; n < r; ++n) {
                const r = e[n];
                r.GetActualHTMLIndex() !== t && (this.#z(t, e.slice(s, n)),
                t = r.GetActualHTMLIndex(),
                s = n)
            }
            s < n && this.#z(t, e.slice(s, n))
        }
        #z(e, t) {
            if (t.length <= 1)
                return;
            if (e >= this.#d.length)
                return;
            const s = t.map(e => e.GetElement())
              , n = new Set(s)
              , r = this.GetHTMLWrapElement(e)
              , i = Array.from(r.children).filter(e => n.has(e));
            for (let e = 0, t = 0, n = s.length; e < n; ++e) {
                const n = s[e]
                  , a = i[t];
                n === a ? ++t : r["moveBefore"] ? r["moveBefore"](n, a) : r.insertBefore(n, a)
            }
        }
        _OnLocalRuntimeMessageFromDOM(e) {
            if (this.#e)
                throw new Error("not available in worker mode");
            this.#i["_OnMessageFromDOM"](e)
        }
        PostToRuntimeComponent(e, t, s, n, r) {
            this.#t.postMessage({
                "type": "event",
                "component": e,
                "handler": t,
        ...
        PostToRuntimeComponentAsync(e, t, s, n, r) {
            const i = p++
              , a = new Promise( (e, t) => {
                c.set(i, {
                    resolve: e,
        ...
        "_OnMessageFromRuntime"(e) {
            ...
            if ("result" === t)
                ...
                this.#$();
            else if ("alert-error" === t)
                this.#G(),
                alert(e["message"]);
            else {
                if ("creating-runtime" !== t)
        ...
        #Z(e) {
            const t = e["component"]
              ...
              , i = d.get(t);
            if (!i)
                return void console.warn(`[DOM] No event handlers for component '${t}'`);
            const a = i.get(s);
            if (!a)
                return void console.warn(`[DOM] No handler '${s}' for component '${t}'`);
            let o = null;
            try {
                o = a(n)
            } catch (e) {
                return console.error(`Exception in '${t}' handler '${s}':`, e),
                void (null !== r && this.#q(r, !1, "" + e))
            }
            if (null === r)
                return o;
            o && o.then ? o.then(e => this.#q(r, !0, e)).catch(e => {
                console.error(`Rejection from '${t}' handler '${s}':`, e),
                this.#q(r, !1, "" + e)
            }
            ) : this.#q(r, !0, o)
        }
        #q(e, t, s) {
            let n;
            s && s["transferables"] && (n = s["transferables"]),
            this.#t.postMessage({
                "type": "result",
                "responseId": e,
                "isOk": t,
        ...
        #J(e) {
            const t = e["responseId"]
              ...
              , n = e["result"]
              , r = c.get(t);
            s ? r.resolve(n) : r.reject(n),
            c.delete(t)
        }
        AddRuntimeComponentMessageHandler(e, t, s) {
            let n = d.get(e);
            if (n || (n = new Map,
            d.set(e, n)),
            n.has(t))
        ...
        static IsDocumentFullscreen() {
            return !!(document["fullscreenElement"] || document["webkitFullscreenElement"] || document["mozFullScreenElement"] || w)
        }
        ...
        _RemoveRAFCallback(e) {
            this.#b.delete(e),
            0 === this.#b.size && this._CancelAnimationFrame()
        }
        _RequestAnimationFrame() {
            -1 === this.#g && this.#b.size > 0 && (this.#g = requestAnimationFrame(this.#f))
        }
        _CancelAnimationFrame() {
            -1 !== this.#g && (cancelAnimationFrame(this.#g),
            this.#g = -1)
        }
        #v() {
            this.#g = -1;
        ...
        TryPlayMedia(e) {
            this.#o.TryPlayMedia(e)
        }
        RemovePendingPlay(e) {
            this.#o.RemovePendingPlay(e)
        }
        _PlayPendingMedia() {
            this.#o._PlayPendingMedia()
        }
        SetSilent(e) {
            this.#o.SetSilent(e)
        }
        ...
        async #V(e) {
            if ("playable-ad-single-file" === this.#C) {
                if (this._localFileStrings.hasOwnProperty(e))
                    return {
                        isStringSrc: !0,
                        str: this._localFileStrings[e]
                    };
                if (this._localFileBlobs.hasOwnProperty(e))
                    return URL.createObjectURL(this._localFileBlobs[e]);
                throw new Error("missing script: " + e)
            }
            return e
        }
        ...
        #X(e) {
            const t = this.#K();
            return new Promise( (s, n) => t["checkPermission"](this.#N(t, e), e => s(!!e["hasPermission"]), n))
        }
        #Y(e) {
            const t = this.#K();
            return new Promise( (s, n) => t["requestPermission"](this.#N(t, e), e => s(!!e["hasPermission"]), n))
        }
        ...
        #I(e, t) {
            if ("object" != typeof e || !e)
                return void console.warn("Unknown wrapper message: ", e);
            const s = e["type"];
            if ("directory-handles" === s)
                this.#R = t;
            else if ("wrapper-init-response" === s)
                this.#M(e),
                this.#M = null;
            else if ("fullscreen-change" === s)
                n._SetWrapperIsFullscreenFlag(e["isFullscreen"]),
                this.#o._OnFullscreenChange();
            else if ("log-to-console" === s)
                switch (e["logType"]) {
                case "error":
                ...
                this.PostToRuntimeComponent("runtime", e[s], null, {
                    "dispatchRuntimeEvent": !0,
                    "dispatchUserScriptEvent": !0
                })
            } else
                "extension-message" === s ? this.PostToRuntimeComponent("runtime", "wrapper-extension-message", e) : console.warn("Unknown wrapper message: ", e)
        }
        #W(e) {
        ...
        #H(e) {
            this._SendWrapperMessage(e)
        }
        _SendWrapperMessage(e) {
            this.IsAnyWebView2Wrapper() ? window["chrome"]["webview"]["postMessage"](JSON.stringify(e)) : "macos-wkwebview" === this.#C ? window["webkit"]["messageHandlers"]["C3Wrapper"]["postMessage"](JSON.stringify(e)) : "linux-cef" === this.#C && window["c3_linux_cef_sendmessage"](JSON.stringify(e))
        }
        #k() {
            window.moveTo = (e, t) => {
                this._SendWrapperMessage({
        ...
        async #_() {
            const e = [];
            for (const [t,s] of Object.entries(this._localFileBlobs))
                e.push(this.#Q(t, s));
            await Promise.all(e)
        }
        async #Q(e, t) {
            if ("object" == typeof t)
                ...
                let s = await this.#ee(t);
                s || (s = this.#te(t)),
                this._localFileBlobs[e] = s
            }
        }
        async #ee(e) {
            try {
        ...
        #te(e) {
            const t = this.#se(e);
            return this.#ne(t.data, t.mime_type)
        }
        #se(e) {
            const t = e.indexOf(",");
            if (t < 0)
                throw new URIError("expected comma in data: uri");
            const s = e.substring(5, t)
              , n = e.substring(t + 1)
              , r = s.split(";")
              , i = r[0] || ""
              , a = r[1]
              , o = r[2];
            let l;
            return l = "base64" === a || "base64" === o ? atob(n) : decodeURIComponent(n),
            {
                mime_type: i,
                data: l
            }
        ...
        #ne(e, t) {
            let s, n, r = e.length, i = r >> 2, a = new Uint8Array(r), o = new Uint32Array(a.buffer,0,i);
            for (s = 0,
            n = 0; s < i; ++s)
                o[s] = e.charCodeAt(n++) | e.charCodeAt(n++) << 8 | e.charCodeAt(n++) << 16 | e.charCodeAt(n++) << 24;
            let l = 3 & r;
            for (; l--; )
                a[n] = e.charCodeAt(n),
                ++n;
...
"use strict";
{
    ...
    const n = new Map([["OSLeft", "MetaLeft"], ["OSRight", "MetaRight"]])
      , s = {
        "dispatchRuntimeEvent": !0,
        "dispatchUserScriptEvent": !0
    }
    ...
    const l = new Set(["input", "textarea", "datalist", "select"]);
    const c = new Set(["canvas", "body", "html"]);
    function h(e) {
        if (!e.target.tagName)
            return;
        const t = e.target.tagName.toLowerCase();
        c.has(t) && e.preventDefault()
    }
    function u(e) {
    ...
    ,
    self["C3_RasterSvgImageBlob"] = async function(e, t, i, n, s) {
        const o = await r(e)
          , a = document.createElement("canvas");
        a.width = n,
        a.height = s;
        return a.getContext("2d").drawImage(o, 0, 0, t, i),
        a
    }
    ;
    let g = !1;
    ...
    const _ = "runtime"
      , w = class extends self.DOMHandler {
        constructor(e) {
            ...
            this._lastWindowWidth = e._GetWindowInnerWidth(),
            this._lastWindowHeight = e._GetWindowInnerHeight(),
            this._virtualKeyboardHeight = 0,
            this._vkTranslateYOffset = 0,
            e.AddRuntimeComponentMessageHandler("runtime", "invoke-download", e => this._OnInvokeDownload(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "load-webfonts", e => this._OnLoadWebFonts(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "raster-svg-image", e => this._OnRasterSvgImage(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "get-svg-image-size", e => this._OnGetSvgImageSize(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "set-target-orientation", e => this._OnSetTargetOrientation(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "register-sw", () => this._OnRegisterSW()),
            e.AddRuntimeComponentMessageHandler("runtime", "post-to-debugger", e => this._OnPostToDebugger(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "go-to-script", e => this._OnPostToDebugger(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "before-start-ticking", () => this._OnBeforeStartTicking()),
            e.AddRuntimeComponentMessageHandler("runtime", "debug-highlight", e => this._OnDebugHighlight(e)),
            ...
            e.AddRuntimeComponentMessageHandler("runtime", "add-stylesheet", e => this._OnAddStylesheet(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "script-create-worker", e => this._OnScriptCreateWorker(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "alert", e => this._OnAlert(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "screen-reader-text", e => this._OnScreenReaderTextEvent(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "hide-cordova-splash", () => this._OnHideCordovaSplash()),
            e.AddRuntimeComponentMessageHandler("runtime", "set-exporting-to-video", e => this._SetExportingToVideo(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "export-to-video-progress", e => this._OnExportVideoProgress(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "exported-to-video", e => this._OnExportedToVideo(e)),
            e.AddRuntimeComponentMessageHandler("runtime", "exported-to-image-sequence", e => this._OnExportedToImageSequence(e));
            const t = new Set(["input", "textarea", "datalist"]);
            if (window.addEventListener("contextmenu", e => {
                const i = e.target
                  , n = i.tagName.toLowerCase();
                t.has(n) || d(i) || e.preventDefault()
            }
            ),
            window.addEventListener("selectstart", h),
            window.addEventListener("gesturehold", h),
            ...
            e.IsiOSWebView()) {
                ...
                window["visualViewport"].addEventListener("resize", () => {
                    ...
                    t > e && (document.scrollingElement.scrollTop = 0,
        ...
        _OnBeforeStartTicking() {
            ...
            document.addEventListener("resume", () => this._OnVisibilityChange(!1))) : document.addEventListener("visibilitychange", () => this._OnVisibilityChange("hidden" === document.visibilityState)),
            this._pageVisibilityIsHidden = !("hidden" !== document.visibilityState && !g),
            {
        ...
        Attach() {
            ...
            window.addEventListener("focusin", e => {
                var t;
                t = e.target,
                (l.has(t.tagName.toLowerCase()) || d(t)) && this._PostRuntimeEvent("keyboard-blur")
            }
            ),
            window.addEventListener("keydown", e => this._OnKeyEvent("keydown", e)),
            window.addEventListener("keyup", e => this._OnKeyEvent("keyup", e)),
            window.addEventListener("mousedown", e => this._OnMouseEvent("mousedown", e, o)),
            window.addEventListener("mousemove", e => this._OnMouseEvent("mousemove", e, o)),
            window.addEventListener("mouseup", e => this._OnMouseEvent("mouseup", e, o)),
            window.addEventListener("dblclick", e => this._OnMouseEvent("dblclick", e, s)),
            window.addEventListener("wheel", e => this._OnMouseWheelEvent("wheel", e, s)),
            window.addEventListener("pointerdown", e => {
                this._HandlePointerDownFocus(e),
                this._OnPointerEvent("pointerdown", e)
            }
            ),
            this._iRuntime.UsesWorker() && void 0 !== window["onpointerrawupdate"] && self === self.top && !t ? window.addEventListener("pointerrawupdate", e => this._OnPointerRawUpdate(e)) : window.addEventListener("pointermove", e => this._OnPointerEvent("pointermove", e)),
            ...
            this._iRuntime.IsiOSWebView() && (document.scrollingElement.scrollTop = 0,
            document.scrollingElement.scrollLeft = 0)
        }
        _OnAndroidVirtualKeyboardChange(e, t) {
            ...
            t > 0) {
                ...
                if (i) {
                    ...
                    s > t && (s = t),
                    s < 0 && (s = 0),
                    s > 0 && (document.body.style.position = "absolute",
                    document.body.style.overflow = "visible",
                    document.body.style.transform = `translateY(${-s}px)`,
                    this._vkTranslateYOffset = s)
                }
            }
        }
        ...
        _OnWindowResize() {
            if (this._isExportToVideo)
                return;
            if (!this._enableWindowResizeEvent)
                return;
            const t = this._GetWindowInnerWidth()
              , i = this._GetWindowInnerHeight();
            if (this._iRuntime.IsAndroidWebView())
                if (this._enableAndroidVKDetection) {
                    if (this._iRuntime.CanDoAndroidVirtualKeyboardDetection() && this._lastWindowWidth === t && i < this._lastWindowHeight)
                        return this._virtualKeyboardHeight = this._lastWindowHeight - i,
                        void this._OnAndroidVirtualKeyboardChange(this._lastWindowHeight, this._virtualKeyboardHeight);
                    this._virtualKeyboardHeight > 0 && (this._virtualKeyboardHeight = 0,
                    this._OnAndroidVirtualKeyboardChange(i, this._virtualKeyboardHeight)),
                    this._lastWindowWidth = t,
                    this._lastWindowHeight = i
                } else
                    this._lastWindowWidth = t,
                    this._lastWindowHeight = i;
            ...
            this._OnSimulatedResize(t, i, 0))
        }
        _ScheduleSimulatedResize(e, t, i) {
            -1 !== this._simulatedResizeTimerId && clearTimeout(this._simulatedResizeTimerId),
            this._simulatedResizeTimerId = setTimeout( () => this._OnSimulatedResize(e, t, i), 48)
        }
        _OnSimulatedResize(t, i, n) {
            const s = this._GetWindowInnerWidth()
              , o = this._GetWindowInnerHeight();
            this._simulatedResizeTimerId = -1,
            s != t || o != i ? this.PostToRuntime("window-resize", {
                "innerWidth": s,
                "innerHeight": o,
                "devicePixelRatio": window.devicePixelRatio,
                "isFullscreen": e.IsDocumentFullscreen(),
                "cssDisplayMode": this._iRuntime.GetCssDisplayMode()
            }) : n < 10 && this._ScheduleSimulatedResize(s, o, n + 1)
        }
        ...
        _TrySetTargetOrientation() {
            ...
            if (screen["orientation"] && screen["orientation"]["lock"])
                screen["orientation"]["lock"](e).catch(e => console.warn("[Construct] Failed to lock orientation: ", e));
            else
                try {
                    let t = !1;
        ...
        _OnFullscreenChange() {
            ...
            t && "any" !== this._targetOrientation && this._TrySetTargetOrientation(),
        ...
        _OnFullscreenError(t) {
            console.warn("[Construct] Fullscreen request failed: ", t),
        ...
        _OnVisibilityChange(e) {
            if (this._pageVisibilityIsHidden !== e && (this._pageVisibilityIsHidden = e,
        ...
        _OnKeyEvent(e, t) {
            if (void 0 === t.key)
                return;
            if ("Backspace" === t.key && h(t),
            this._isExportToVideo)
                return;
            const i = n.get(t.code) || t.code;
        ...
        _OnMouseEvent(e, t, n) {
            this._isExportToVideo || i(t) || this._PostToRuntimeMaybeSync(e, {
                "button": t.button,
                "buttons": t.buttons,
                "clientX": t.clientX,
                "clientY": t.clientY + this._vkTranslateYOffset,
        ...
        _OnPointerEvent(e, t) {
            if (this._isExportToVideo)
                return;
            let i = 0;
            "mouse" === t.pointerType && (i = this._mousePointerLastButtons),
            this._PostToRuntimeMaybeSync(e, {
            ...
            "mouse" === t.pointerType && (this._mousePointerLastButtons = t.buttons)
        }
            ...
            if (!this._isExportToVideo)
                for (let i = 0, n = t.changedTouches.length; i < n; ++i) {
                    ...
                    this._PostToRuntimeMaybeSync(e, {
                        ...
                        "lastButtons": 0,
                        "clientX": n.clientX,
                        "clientY": n.clientY + this._vkTranslateYOffset,
                        "pageX": n.pageX,
                        "pageY": n.pageY + this._vkTranslateYOffset,
                        "movementX": t.movementX || 0,
                        "movementY": t.movementY || 0,
        ...
        async _OnRasterSvgImage(e) {
            const t = e["blob"]
              ...
              , o = e["surfaceHeight"]
              , a = e["imageBitmapOpts"]
              , r = await self["C3_RasterSvgImageBlob"](t, i, n, s, o);
            let d;
        ...
        _OnDebugHighlight(e) {
            ...
            t.style.display = "",
            t.style.left = e["left"] - 1 + "px",
            t.style.top = e["top"] - 1 + "px",
            t.style.width = e["width"] + 2 + "px",
            t.style.height = e["height"] + 2 + "px",
        ...
        _OnScreenReaderTextEvent(e) {
            ...
            if ("create" === t) {
                ...
                this._screenReaderTextWrap.appendChild(t)
            } else if ("update" === t) {
                ...
                console.warn(`[Construct] Unknown screen reader text update '${t}'`)
        }
        _SetExportingToVideo(e) {
            this._isExportToVideo = !0;
            const t = document.createElement("h1");
            t.id = "exportToVideoMessage",
    ...
    ;
    e.AddDOMHandlerClass(w)
}

// workers/jobSchedulerDom.js
"use strict";
{
    const t = "dispatchworker.js"
      , r = "jobworker.js";
    self.JobSchedulerDOM = class {
        ...
        async Init() {
            ...
            const r = this._runtimeInterface.GetScriptFolder() + t;
            this._dispatchWorker = await this._runtimeInterface.CreateWorker(r, {
                name: "DispatchWorker"
            });
        ...
        async _CreateJobWorker() {
            const t = this._jobWorkers.length
              , e = this._runtimeInterface.GetScriptFolder() + r
              , o = await this._runtimeInterface.CreateWorker(e, {
                name: "JobWorker" + t
            })
...
let k = [{
    ...
    scriptSrc: "https://www.apkgosu.fun/ad/sdk/sdk.js",
    hasAds: !0,
    noInterstitial: !1,
    noRewarded: !1,
    hasBanner: !1,
    enableOnlyInProduction: !1,
    implementation: {
        init(e=!1, t) {
        ...
        setUpEventListeners(e, t) {
            let n = 0
              , d = t.poki_maxBeacons || 6
              , l = t.poki_beaconInterval || 60;
            const m = c => {
                if (t.poki_gameId && t.poki_doBeacon) {
                    let u = Date.now();
                    if (u - n < l * 1e3 || d <= 0)
                        return;
                    n = u,
                    d--,
                    navigator.sendBeacon(`https://leveldata.poki.io/${c}`, t.poki_gameId)
                }
            }
            ;
            p("loadingStart", () => {
            ...
            p("loadingEnd", () => {
                o.gameLoadingFinished(),
                m("loadingEnd")
            }
            ),
            p("gameplayStart", () => {
                m("gameplayStart"),
                !i.gameplayStarted && (i.gameplayStarted = !0,
                o.gameplayStart())
            }
            ),
            p("gameplayStop", () => {
                m("gameplayStop"),
                i.gameplayStarted && (i.gameplayStarted = !1,
                o.gameplayStop())
            }
            ),
            p("interstitial", () => {
                m("interstitial"),
                s("adStarted", i.lastRequestedAd),
                o.commercialBreak().then( () => {
                    s("interstitialEnd", !0)
                }
                )
            }
...
const r = {
    ...
    init(e, t=!1, n={}) {
        return new Promise(async d => {
            if (a = k.find(l => l.name.toLowerCase() === e.toLowerCase()),
            a)
                if (S = !0,
                a.enableOnlyInProduction && t)
                    S = !1,
                    d();
                else if (a.implementation.preInit && await a.implementation.preInit(t, n),
                a.scriptSrc) {
                    const l = async () => {
                        o = a.sdk,
                        a.implementation.setUpEventListeners(t, n),
                        a.implementation.init && await a.implementation.init(t, n),
                        r.loadingStart(),
                        d()
                    }
                    ;
                    a.scriptSrc instanceof Array ? (await Promise.all(a.scriptSrc.map(m => new Promise(c => {
                        f(m, a.name + "-jssdk", c)
                    }
                    ))),
                    l()) : f(a.scriptSrc, a.name + "-jssdk", l)
                } else
                    d();
            else
                d()
    ...
    interstitial() {
        return i.lastRequestedAd = "interstitial",
        !a || !a.hasAds || a.noInterstitial ? (s("adStarted", i.lastRequestedAd),
        Promise.resolve(!1)) : new Promise(e => {
            let t = i.gameplayStarted;
            t && r.gameplayStop(),
            s("interstitial"),
            y("interstitialEnd", (...n) => {
                t && r.gameplayStart(),
    ...
    rewarded() {
        return i.lastRequestedAd = "rewarded",
        !a || !a.hasAds || a.noRewarded ? (s("adStarted", i.lastRequestedAd),
        Promise.resolve(!0)) : new Promise(e => {
            let t = i.gameplayStarted;
            t && r.gameplayStop(),
            s("rewarded"),
            y("rewardedEnd", (...n) => {
    ...
    hasAds() {
        return a && a.hasAds ? 1 : 0
    },
    hasInterstitialAds() {
        return a && a.hasAds && !a.noInterstitial ? 1 : 0
    },
    hasRewardedAds() {
        return a && a.hasAds && !a.noRewarded ? 1 : 0
    },
    analyticsEvent({category: e, action: t, label: n}) {
        s("analyticsEvent", e, t, n)
    }
};
function E(e) {
    return class extends e {
        constructor(t) {
            super(t),
            this.AddRuntimeMessageHandlers([["Init", this.Init.bind(this)], ["LoadingEnd", r.loadingEnd], ["GameplayStart", r.gameplayStart], ["GameplayStop", r.gameplayStop], ["Interstitial", r.interstitial], ["Rewarded", r.rewarded], ["AnalyticsEvent", r.analyticsEvent]])
        }
        async Init({debug: t, config: n}) {
            return r.onAdStarted( () => {
            ...
            await r.init("Poki", t, n),
            {
                enabled: r.enabled,
                hasAds: r.hasAds(),
                hasInterstitialAds: r.hasInterstitialAds(),
                hasRewardedAds: r.hasRewardedAds()
            }
        }
    }
}
self.RuntimeInterface.AddDOMHandlerClass(E(class extends self.DOMHandler {
    constructor(e) {
        super(e, A)
    }
}
    ...
    const e = "touch"
      , t = class extends self.DOMHandler {
        ...
        async _OnRequestPermission(e) {
            const t = e["type"];
            let s = !0;
            0 === t ? s = await this._RequestOrientationPermission() : 1 === t && (s = await this._RequestMotionPermission()),
...
"use strict";
{
    ...
    const i = "browser"
      , r = class extends self.DOMHandler {
        constructor(e) {
            ...
            window.addEventListener("fullscreenerror", () => this._OnFullscreenError()),
            window.addEventListener("webkitfullscreenerror", () => this._OnFullscreenError()),
            window.addEventListener("mozfullscreenerror", () => this._OnFullscreenError()),
            this._beforeunload_handler = e => e.preventDefault(),
            document.addEventListener("backbutton", () => this._OnCordovaBackButton())
        }
        Attach() {
            e ? this._OnBeforeInstallPrompt() : t = this,
        ...
        async _OnRequestInstall() {
            ...
            try {
                e["prompt"]();
                return {
                    "result": (await e["userChoice"])["outcome"]
                }
            } catch (e) {
        ...
        _OnSetFocus(e) {
            e["isFocus"] ? window.focus() : window.blur()
        }
        _OnVibrate(e) {
        ...
        _OnLockOrientation(e) {
            ...
            if (screen["orientation"] && screen["orientation"]["lock"])
                screen["orientation"]["lock"](t).catch(e => console.warn("[Construct] Failed to lock orientation: ", e));
            else
                try {
                    let e = !1;
        ...
        _OnNavigate(e) {
            ...
            if ("back" === t)
                ...
                const t = e["url"]
                  , n = e["target"]
                  , o = e["exportType"];
                self["cordova"] && self["cordova"]["InAppBrowser"] ? self["cordova"]["InAppBrowser"]["open"](t, "_system") : "preview" === o || "macos-wkwebiew" === o || "linux-cef" === o || this._iRuntime.IsAnyWebView2Wrapper() ? window.open(t, "_blank") : this._isConstructArcade || (2 === n ? window.top.location = t : 1 === n ? window.parent.location = t : window.location = t)
            } else if ("new-window" === t) {
                const t = e["url"]
                  , n = e["tag"];
                self["cordova"] && self["cordova"]["InAppBrowser"] ? self["cordova"]["InAppBrowser"]["open"](t, "_system") : window.open(t, n)
            }
        }
        _OnRequestFullscreen(e) {
            if (this._iRuntime.IsAnyWebView2Wrapper() || "macos-wkwebview" === this._exportType || "linux-cef" === this._exportType)
                ...
                let r;
                i["requestFullscreen"] ? r = i["requestFullscreen"](t) : i["mozRequestFullScreen"] ? r = i["mozRequestFullScreen"](t) : i["msRequestFullscreen"] ? r = i["msRequestFullscreen"](t) : i["webkitRequestFullScreen"] && (r = void 0 !== Element["ALLOW_KEYBOARD_INPUT"] ? i["webkitRequestFullScreen"](Element["ALLOW_KEYBOARD_INPUT"]) : i["webkitRequestFullScreen"]()),
                r instanceof Promise && r.catch(o)
            }
        }
        _OnFullscreenError() {
            this.PostToRuntime("fullscreenerror")
        ...
        _OnExitFullscreen() {
            if (this._iRuntime.IsAnyWebView2Wrapper() || "macos-wkwebview" === this._exportType || "linux-cef" === this._exportType)
                ...
                let e;
                document["exitFullscreen"] ? e = document["exitFullscreen"]() : document["mozCancelFullScreen"] ? e = document["mozCancelFullScreen"]() : document["msExitFullscreen"] ? e = document["msExitFullscreen"]() : document["webkitCancelFullScreen"] && (e = document["webkitCancelFullScreen"]()),
        ...
        _OnSetDocumentCSSStyle(e) {
            ...
            try {
                const e = n(i, r);
                for (const n of e)
                    t.startsWith("--") ? n.style.setProperty(t, o) : n.style[t] = o
            } catch (e) {
    ...
    ;
    self.RuntimeInterface.AddDOMHandlerClass(r)
}
...
"use strict";
{
    const t = "localstorage"
      , s = class extends self.DOMHandler {
        constructor(s) {
            super(s, t),
            this.AddRuntimeMessageHandlers([["init", () => this._Init()], ["request-persistent", () => this._OnRequestPersistent()]])
        }
        async _Init() {
            let t = !1;
            try {
                ...
                t = !1,
                console.warn("[Construct] Error checking storage persisted state: ", s)
            }
            return {
                "isPersistent": t
            }
...
"use strict";
{
    ...
    self.AudioDOMHandler = class extends self.DOMHandler {
        ...
        async _CreateAudioContext(t) {
            ...
            this._iRuntime.IsExportingToVideo()) {
                ...
                this._audioContext = new OfflineAudioContext({
                    "numberOfChannels": 2,
                    "sampleRate": t,
                    "length": Math.ceil(this._iRuntime.GetExportToVideoDuration() * t)
                })
            } else
                ...
                this._audioContext.onstatechange = () => {
                    "running" === this._audioContext.state ? this._DetachUnblockEvents() : this._AttachUnblockEvents(),
            ...
            const e = t["listenerPos"];
            this._lastListenerPos[0] = e[0],
            this._lastListenerPos[1] = e[1],
            this._lastListenerPos[2] = e[2],
            ...
            try {
                await Promise.all(t["preloadList"].map(t => this._GetAudioBuffer(t["originalUrl"], t["url"], t["type"], !1)))
            } catch (t) {
        ...
        _AttachUnblockEvents() {
            this._hasAttachedUnblockEvents || (window.addEventListener("pointerup", this._unblockFunc, !0),
            ...
            this._iRuntime.IsiOSWebView() || document.addEventListener("visibilitychange", this._unblockFunc),
            this._hasAttachedUnblockEvents = !0)
        }
        _DetachUnblockEvents() {
            this._hasAttachedUnblockEvents && (window.removeEventListener("pointerup", this._unblockFunc, !0),
            ...
            this._iRuntime.IsiOSWebView() || document.removeEventListener("visibilitychange", this._unblockFunc),
            this._hasAttachedUnblockEvents = !1)
        }
        ...
        _MatchTagLists(t, e) {
            for (const s of e) {
                let e = !1;
                for (const i of t)
                    if (self.AudioDOMHandler.EqualsNoCase(i, s)) {
                        e = !0;
                        break
                    }
                if (!e)
        ...
        GetDestinationForTag(t) {
            const e = this._effects.get(t.toLowerCase());
            return e ? e[0].GetInputNode() : this.GetDestinationNode()
        }
        AddEffectForTag(t, e) {
            t = t.toLowerCase();
            let s = this._effects.get(t);
            s || (s = [],
            this._effects.set(t, s)),
            e._SetIndex(s.length),
            e._SetTag(t),
            s.push(e),
        ...
        _ReconnectEffects(t) {
            t = t.toLowerCase();
            let e = this.GetDestinationNode();
            const s = this._effects.get(t);
            if (s && s.length) {
                e = s[0].GetInputNode();
                for (let t = 0, e = s.length; t < e; ++t) {
                    const i = s[t];
                    t + 1 === e ? i.ConnectTo(this.GetDestinationNode()) : i.ConnectTo(s[t + 1].GetInputNode())
                }
            }
            for (const s of this.audioInstancesByEffectTag(t))
                s.Reconnect(e);
            this._microphoneSource && this._microphoneTag === t && (this._microphoneSource["disconnect"](),
            this._microphoneSource["connect"](e))
        }
        GetMasterVolume() {
        ...
        TryPlayMedia(t) {
            this._iRuntime.TryPlayMedia(t)
        }
        RemovePendingPlay(t) {
            this._iRuntime.RemovePendingPlay(t)
        }
        ReleaseInstancesForBuffer(t) {
            let e = 0;
            for (let s = 0, i = this._audioInstances.length; s < i; ++s) {
                const i = this._audioInstances[s];
                this._audioInstances[e] = i,
                i.GetBuffer() === t ? i.Release() : ++e
            }
            this._audioInstances.length = e
        }
        ReleaseAllMusicBuffersExceptLast() {
            let t = 0;
            for (let e = 0, s = this._audioBuffers.length; e < s; ++e) {
                const s = this._audioBuffers[e];
                this._audioBuffers[t] = s,
                s.IsMusic() && s !== this._lastMusicBuffer ? s.Release() : ++t
            }
            this._audioBuffers.length = t
        }
        *audioInstancesMatchingTags(t) {
            if (t.length > 0)
                for (const e of this._audioInstances)
                    this._MatchTagLists(e.GetTags(), t) && (yield e);
            else
                this._lastAudioInstance && !this._lastAudioInstance.HasEnded() && (yield this._lastAudioInstance)
        }
        *audioInstancesByEffectTag(t) {
            if (t)
                for (const e of this._audioInstances)
                    self.AudioDOMHandler.EqualsNoCase(e.GetEffectTag(), t) && (yield e);
            else
                this._lastAudioInstance && !this._lastAudioInstance.HasEnded() && (yield this._lastAudioInstance)
        }
        async _GetAudioBuffer(t, e, s, i, n) {
            for (const t of this._audioBuffers)
                if (t.GetUrl() === e)
                    return i && this._playMusicAsSound && (this._lastMusicBuffer = t),
                    await t.Load(),
                    t;
            if (n)
                return null;
            i && this._playMusicAsSound && this.ReleaseAllMusicBuffersExceptLast();
            const o = self.C3AudioBuffer.Create(this, t, e, s, i);
            return this._audioBuffers.push(o),
            i && this._playMusicAsSound && (this._lastMusicBuffer = o),
            await o.Load(),
            this._loadedAudioUrls.has(t) || (this.PostToRuntime("buffer-metadata", {
                "originalUrl": t,
                "duration": o.GetDuration()
            }),
            this._loadedAudioUrls.add(t)),
            o
        }
        async _GetAudioInstance(t, e, s, i, n) {
            for (const t of this._audioInstances)
                if (t.GetUrl() === e && (t.CanBeRecycled() || n))
                    return n && this._playMusicAsSound && (this._lastMusicBuffer = t.GetBuffer()),
                    t.SetTags(i),
                    t;
            const o = await this._GetAudioBuffer(t, e, s, n);
            if (o.WasReleased())
                throw new Error(`buffer '${t}' unloaded while loading`);
            const a = o.CreateInstance(i);
            return this._audioInstances.push(a),
            a
        }
        _AddPendingTags(t) {
            const e = t.join(" ");
            let s = this._pendingTags.get(e);
            if (!s) {
                let t = null;
                s = {
                    pendingCount: 0,
                ...
                this._pendingTags.set(e, s)
            }
            s.pendingCount++
        }
        _RemovePendingTags(t) {
            const e = t.join(" ")
              , s = this._pendingTags.get(e);
            if (!s)
                throw new Error("expected pending tag");
            s.pendingCount--,
            0 === s.pendingCount && (s.resolve(),
            this._pendingTags.delete(e))
        }
        TagsReady(t) {
            const e = (0 === t.length ? this._lastPlayedTags : t).join(" ")
              , s = this._pendingTags.get(e);
        ...
        Tick() {
            for (const t of this._analysers)
                t.Tick();
            const t = this.GetAudioCurrentTime();
            for (const e of this._audioInstances)
                e.Tick(t);
            const e = this._audioInstances.filter(t => t.IsActive()).map(t => t.GetState());
            this.PostToRuntime("state", {
                "tickCount": this._lastTickCount,
                "outputLatency": this._audioContext["outputLatency"] || 0,
                "audioInstances": e,
                "analysers": [...this._analysers].map(t => t.GetData())
            }),
            0 === e.length && 0 === this._analysers.size && this._StopTicking()
        }
        PostTrigger(t, e, s) {
        ...
        async _Play(t) {
            const e = t["originalUrl"]
              ...
              , d = t["stereoPan"];
            let h = t["off"];
            h > 0 && !t["trueClock"] && (h += this.GetAudioCurrentTime()),
            this._lastPlayedTags = o.slice(0),
            this._AddPendingTags(o);
            try {
                this._lastAudioInstance = await this._GetAudioInstance(e, s, i, o, n),
                u ? (this._lastAudioInstance.SetPannerEnabled(!0),
                this._lastAudioInstance.SetPan(u["x"], u["y"], u["z"], u["angle"], u["innerAngle"], u["outerAngle"], u["outerGain"]),
                u.hasOwnProperty("uid") && this._lastAudioInstance.SetUID(u["uid"])) : "number" == typeof d && 0 !== d ? (this._lastAudioInstance.SetStereoPannerEnabled(!0),
                this._lastAudioInstance.SetStereoPan(d)) : (this._lastAudioInstance.SetPannerEnabled(!1),
                this._lastAudioInstance.SetStereoPannerEnabled(!1)),
                this._lastAudioInstance.Play(a, r, l, h)
            } catch (t) {
                return void console.error("[Construct] Audio: error starting playback: ", t)
            } finally {
                this._RemovePendingTags(o)
            }
            this._StartTicking()
        }
        _Stop(t) {
            const e = t["tags"];
            for (const t of this.audioInstancesMatchingTags(e))
                t.Stop()
        }
        _StopAll() {
            for (const t of this._audioInstances)
        ...
        async _FadeVolume(t) {
            const e = t["tags"]
              ...
              , i = t["duration"]
              , n = t["stopOnEnd"];
            await this.TagsReady(e);
            for (const t of this.audioInstancesMatchingTags(e))
                t.FadeVolume(s, i, n);
            this._MaybeStartTicking()
        }
        _SetMasterVolume(t) {
            this._masterVolume = t["vol"],
        ...
        async _Preload(t) {
            const e = t["originalUrl"]
              ...
              , n = t["isMusic"];
            try {
                await this._GetAudioInstance(e, s, i, "", n)
            } catch (t) {
        ...
        async _Unload(t) {
            const e = t["url"]
              , s = t["type"]
              , i = t["isMusic"]
              , n = await this._GetAudioBuffer("", e, s, i, !0);
            if (!n)
                return;
            n.Release(),
            this._lastMusicBuffer === n && (this._lastMusicBuffer = null);
            const o = this._audioBuffers.indexOf(n);
            -1 !== o && this._audioBuffers.splice(o, 1)
        }
        _UnloadAll() {
            for (const t of this._audioBuffers)
                t.Release(),
                this._lastMusicBuffer === t && (this._lastMusicBuffer = null);
            this._audioBuffers.length = 0
        }
        _SetSuspended(t) {
            const e = t["isSuspended"];
            !e && this._audioContext["resume"] && this._audioContext["resume"]();
            for (const t of this._audioInstances)
                t.SetSuspended(e);
            e && this._audioContext["suspend"] && this._audioContext["suspend"]()
        }
        _OnTick(t) {
            if (this._timeScale = t["timeScale"],
            this._gameTime = t["gameTime"],
            this._lastTickCount = t["tickCount"],
            0 !== this._timeScaleMode)
                for (const t of this._audioInstances)
                    t._UpdatePlaybackRate();
            const e = t["listenerPos"];
            !e || this._lastListenerPos[0] === e[0] && this._lastListenerPos[1] === e[1] && this._lastListenerPos[2] === e[2] || (this._lastListenerPos[0] = e[0],
            this._lastListenerPos[1] = e[1],
            this._lastListenerPos[2] = e[2],
            this._audioContext["listener"]["setPosition"](e[0], e[1], e[2]));
            const s = t["listenerOrientation"];
            if (s && (this._lastListenerOrientation[0] !== s[0] || this._lastListenerOrientation[1] !== s[1] || this._lastListenerOrientation[2] !== s[2] || this._lastListenerOrientation[3] !== s[3] || this._lastListenerOrientation[4] !== s[4] || this._lastListenerOrientation[5] !== s[5])) {
                for (let t = 0; t < 6; ++t)
                    this._lastListenerOrientation[t] = s[t];
                this._audioContext["listener"]["setOrientation"](...this._lastListenerOrientation)
            }
            for (const e of t["instPans"]) {
                const t = e["uid"];
                for (const s of this._audioInstances)
                    s.GetUID() === t && s.SetPanXYZA(e["x"], e["y"], e["z"], e["angle"])
            }
        }
        async _AddEffect(t) {
            const e = t["type"]
              , s = t.hasOwnProperty("tags") ? t["tags"] : [t["tag"]]
              , i = t["params"];
            let n, o;
            if ("convolution" === e)
                try {
                    o = await this._GetAudioBuffer(t["bufferOriginalUrl"], t["bufferUrl"], t["bufferType"], !1)
                } catch (t) {
                    return void console.log("[Construct] Audio: error loading convolution: ", t)
                }
            for (const a of s) {
                if ("filter" === e)
                    n = new self.C3AudioFilterFX(this,...i);
                else if ("delay" === e)
                    n = new self.C3AudioDelayFX(this,...i);
                else if ("convolution" === e)
                    n = new self.C3AudioConvolveFX(this,o.GetAudioBuffer(),...i),
                    n._SetBufferInfo(t["bufferOriginalUrl"], t["bufferUrl"], t["bufferType"]);
                else if ("flanger" === e)
                    n = new self.C3AudioFlangerFX(this,...i);
                else if ("phaser" === e)
                    n = new self.C3AudioPhaserFX(this,...i);
                else if ("gain" === e)
                    n = new self.C3AudioGainFX(this,...i);
                else if ("stereopan" === e)
                    n = new self.C3AudioStereoPanFX(this,...i);
                else if ("tremolo" === e)
                    n = new self.C3AudioTremoloFX(this,...i);
                else if ("ringmod" === e)
                    n = new self.C3AudioRingModFX(this,...i);
                else if ("distortion" === e)
                    n = new self.C3AudioDistortionFX(this,...i);
                else if ("compressor" === e)
                    n = new self.C3AudioCompressorFX(this,...i);
                else {
                    if ("analyser" !== e)
                        throw new Error("invalid effect type");
                    n = new self.C3AudioAnalyserFX(this,...i)
                }
                this.AddEffectForTag(a, n)
            }
        ...
        _SetEffectParam(t) {
            ...
            for (const t of e) {
                const e = this._effects.get(t.toLowerCase());
                !e || s < 0 || s >= e.length || e[s].SetParam(i, n, o, a)
            }
            this._PostUpdatedFxState()
        }
        _RemoveEffects(t) {
        ...
        _AddAnalyser(t) {
            this._analysers.add(t),
            this._MaybeStartTicking()
        }
        _RemoveAnalyser(t) {
            this._analysers.delete(t)
        }
        _PostUpdatedFxState() {
        ...
        _DoPostUpdatedFxState() {
            const t = {};
            for (const [e,s] of this._effects)
                t[e] = s.map(t => t.GetState());
            this.PostToRuntime("fxstate", {
                "fxstate": t
            }),
            this._isPendingPostFxState = !1
        }
        async _OnLoadState(t) {
            const e = t["saveLoadMode"];
            if (3 !== e) {
                const t = [];
                for (const s of this._audioInstances)
                    s.IsMusic() && 1 === e || !s.IsMusic() && 2 === e ? t.push(s) : s.Release();
                this._audioInstances = t
            }
            ...
            this._lastListenerPos[0] = s[0],
            this._lastListenerPos[1] = s[1],
            this._lastListenerPos[2] = s[2],
            this._audioContext["listener"]["setPosition"](s[0], s[1], s[2]);
            const i = t["listenerOrientation"];
            if (Array.isArray(i)) {
                for (let t = 0; t < 6; ++t)
                    this._lastListenerOrientation[t] = i[t];
                this._audioContext["listener"]["setOrientation"](...this._lastListenerOrientation)
            }
            this._isSilent = t["isSilent"],
            this._iRuntime.SetSilent(this._isSilent),
            this._masterVolume = t["masterVolume"],
            this._destinationNode["gain"]["value"] = this._masterVolume;
            const n = [];
            for (const e of Object.values(t["effects"]))
                n.push(Promise.all(e.map(t => this._AddEffect(t))));
            await Promise.all(n),
            await Promise.all(t["playing"].map(t => this._LoadAudioInstance(t, e))),
            this._MaybeStartTicking()
        }
        async _LoadAudioInstance(t, e) {
            if (3 === e)
                return;
            const s = t["bufferOriginalUrl"]
              , i = t["bufferUrl"]
              , n = t["bufferType"]
              , o = t["isMusic"]
              , a = t["tags"]
            ...
            let d = null;
            try {
                d = await this._GetAudioInstance(s, i, n, a, o)
            } catch (t) {
                return void console.error("[Construct] Audio: error loading audio state: ", t)
            }
            d.LoadPanState(t["pan"]),
            d.LoadStereoPanState(t["stereoPan"]),
            d.Play(r, l, u, 0),
            t["isPlaying"] || d.Pause(),
            d._LoadAdditionalState(t)
        }
        _OnMicrophoneStream(t, e) {
            this._microphoneSource && this._microphoneSource["disconnect"](),
            this._microphoneTag = e.toLowerCase(),
            this._microphoneSource = this._audioContext["createMediaStreamSource"](t),
        ...
        async _OnOfflineRenderAudio(t) {
            try {
                const e = t["time"]
                  , s = this._audioContext["suspend"](e);
                this._hasStartedOfflineRender ? this._audioContext["resume"]() : (this._audioContext["startRendering"]().then(t => this._OnOfflineRenderCompleted(t)).catch(t => this._OnOfflineRenderError(t)),
                this._hasStartedOfflineRender = !0),
                await s
            } catch (t) {
                this._OnOfflineRenderError(t)
            }
        }
        _OnOfflineRenderFinish() {
            this._audioContext["resume"]()
        }
        _OnOfflineRenderCompleted(t) {
            const e = [];
            for (let s = 0, i = t["numberOfChannels"]; s < i; ++s) {
        ...
        _OnOfflineRenderError(t) {
            console.error("[Audio] Offline rendering error: ", t)
        }
        static EqualsNoCase(t, e) {
            return t === e || t.normalize().toLowerCase() === e.normalize().toLowerCase()
        }
        static ToDegrees(e) {
            return e * t
        }
        static DbToLinearNoCap(t) {
            return Math.pow(10, t / 20)
        }
        static DbToLinear(t) {
            return Math.max(Math.min(self.AudioDOMHandler.DbToLinearNoCap(t), 1), 0)
        }
        static LinearToDbNoCap(t) {
            return Math.log(t) / Math.log(10) * 20
        }
        static LinearToDb(t) {
            return self.AudioDOMHandler.LinearToDbNoCap(Math.max(Math.min(t, 1), 0))
        }
        static e4(t, e) {
...
self.C3AudioBuffer = class {
    ...
    static Create(t, e, i, s, o) {
        return !o || t.IsPlayMusicAsSound() ? new self.C3WebAudioBuffer(t,e,i,s,o) : new self.C3Html5AudioBuffer(t,e,i,s,o)
    }
    CreateInstance(t) {
        return "html5" === this._api ? new self.C3Html5AudioInstance(this._audioDomHandler,this,t) : new self.C3WebAudioInstance(this._audioDomHandler,this,t)
    }
    _Load() {}
    Load() {
...
self.C3WebAudioBuffer = class extends self.C3AudioBuffer {
    ...
    async _Fetch() {
        ...
        if ("playable-ad-single-file" === a.GetExportType() && a.IsRelativeURL(this._url)) {
            const t = a["_PlayableAdFetchBlob"](this._url);
            this._audioData = await t.arrayBuffer()
        } else {
            const a = await fetch(this._url);
            if (!a.ok)
                throw new Error(`error fetching audio data: ${a.status} ${a.statusText}`);
    ...
    async _Decode() {
        ...
        const a = this._audioDomHandler.GetAudioContext();
        this._audioBuffer = await a["decodeAudioData"](this._audioData),
        this._audioData = null
    }
    async _Load() {
        try {
...
"use strict";
{
    ...
    self.C3AudioInstance = class {
        constructor(t, n, i) {
            ...
            this._buffer = n,
            this._tags = i,
            this._lastEffectTag = this.GetEffectTag(),
            this._aiId = e++,
            ...
            this._isTimescaled = 1 === s && !this.IsMusic() || 2 === s,
            this._instUid = -1,
            this._fadeEndTime = -1,
        ...
        Release() {
            ...
            this._pannerNode = null),
            this._stereoPannerNode && (this._stereoPannerNode["disconnect"](),
            this._stereoPannerNode = null),
        ...
        SetTags(e) {
            ...
            this._lastEffectTag !== t && (this._lastEffectTag = t,
            this.Reconnect(this.GetDestinationNode()))
        }
        GetTags() {
            return this._tags
        }
        ...
        FadeVolume(e, t, n) {
            if (this.IsMuted())
                return;
            const i = this._gainNode["gain"];
            i["cancelScheduledValues"](0);
            const s = this._audioDomHandler.GetAudioCurrentTime()
              , a = s + t;
            i["setValueAtTime"](i["value"], s),
            i["linearRampToValueAtTime"](e, a),
            this._volume = e,
            this._fadeEndTime = a,
            this._stopOnFadeEnd = n
        }
        _UpdateVolume() {
        ...
        SetPannerEnabled(e) {
            ...
            this._isPannerEnabled ? (this.SetStereoPannerEnabled(!1),
            this._pannerNode || (this._pannerNode = this.GetAudioContext()["createPanner"](),
        ...
        SetPan(e, t, n, i, s, a, o) {
            if (!this._isPannerEnabled)
                return;
            this.SetPanXYZA(e, t, n, i);
            const r = self.AudioDOMHandler.ToDegrees;
            this._pannerConeParams[0] !== r(s) && (this._pannerConeParams[0] = r(s),
            this._pannerNode["coneInnerAngle"] = r(s)),
            this._pannerConeParams[1] !== r(a) && (this._pannerConeParams[1] = r(a),
            this._pannerNode["coneOuterAngle"] = r(a)),
            this._pannerConeParams[2] !== o && (this._pannerConeParams[2] = o,
            this._pannerNode["coneOuterGain"] = o)
        }
        SetPanXYZA(e, t, n, i) {
            if (!this._isPannerEnabled)
                return;
            const s = this._pannerPosition
              , a = this._pannerOrientation
              , o = Math.cos(i)
              , r = Math.sin(i);
            s[0] === e && s[1] === t && s[2] === n || (s[0] = e,
            s[1] = t,
            s[2] = n,
            this._pannerNode["setPosition"](...s)),
            a[0] === o && a[1] === r && 0 === a[2] || (a[0] = o,
            a[1] = r,
            a[2] = 0,
            this._pannerNode["setOrientation"](...a))
        }
        SetStereoPannerEnabled(e) {
            e = !!e,
            this._isStereoPannerEnabled !== e && (this._isStereoPannerEnabled = e,
            this._isStereoPannerEnabled ? (this.SetPannerEnabled(!1),
            this._stereoPannerNode = this.GetAudioContext()["createStereoPanner"](),
            this._gainNode["disconnect"](),
            this._gainNode["connect"](this._stereoPannerNode),
            this._stereoPannerNode["connect"](this.GetDestinationNode())) : (this._stereoPannerNode["disconnect"](),
            this._stereoPannerNode = null,
            this._gainNode["disconnect"](),
            this._gainNode["connect"](this.GetDestinationNode())))
        }
        SetStereoPan(e) {
            this._isStereoPannerEnabled && this._stereoPan !== e && (this._stereoPannerNode["pan"]["value"] = e,
            this._stereoPan = e)
        }
        SetUID(e) {
            this._instUid = e
        ...
        LoadPanState(e) {
            ...
            const t = this._pannerNode
              , n = e["pos"];
            this._pannerPosition[0] = n[0],
            this._pannerPosition[1] = n[1],
            this._pannerPosition[2] = n[2];
            const i = e["orient"];
            this._pannerOrientation[0] = i[0],
            this._pannerOrientation[1] = i[1],
            this._pannerOrientation[2] = i[2],
            t["setPosition"](...this._pannerPosition),
            t["setOrientation"](...this._pannerOrientation),
            this._pannerConeParams[0] = e["cia"],
            this._pannerConeParams[1] = e["coa"],
            this._pannerConeParams[2] = e["cog"],
            t["coneInnerAngle"] = e["cia"],
            t["coneOuterAngle"] = e["coa"],
            t["coneOuterGain"] = e["cog"],
            this._instUid = e["uid"]
        }
        GetStereoPanState() {
        ...
        LoadStereoPanState(e) {
            "number" == typeof e ? (this.SetStereoPannerEnabled(!0),
            this.SetStereoPan(e)) : this.SetStereoPannerEnabled(!1)
        }
    }
}
...
self.C3Html5AudioInstance = class extends self.C3AudioInstance {
    ...
    GetPlaybackTime() {
        ...
        return this._isLooping || (e = Math.min(e, this.GetDuration())),
        e
    }
    Play(e, t, i, s) {
        const d = this.GetAudioElement();
        if (1 !== d.playbackRate && (d.playbackRate = 1),
        d.loop !== e && (d.loop = e),
        this.SetVolume(t),
        this._isMuted = !1,
        d.muted && (d.muted = !1),
...
self.C3WebAudioInstance = class extends self.C3AudioInstance {
    ...
    _ReleaseBufferSource() {
        this._bufferSource && (this._bufferSource["onended"] = null,
    ...
    GetPlaybackTime() {
        let e = 0;
        return e = this._isPaused ? this._resumePosition : this._playFromSeekPos + (this.GetCurrentTime() - this._playStartTime) * this._playbackRate,
        this._isLooping || (e = Math.min(e, this.GetDuration())),
        e
    }
    Play(e, t, s, i) {
        this._isMuted = !1,
    ...
    Pause() {
        this._isPaused || this._isStopped || this.HasEnded() || (this._resumePosition = this.GetPlaybackTime(),
...
"use strict";
{
    class e {
        ...
        SetAudioParam(e, t, i, s) {
            if (e["cancelScheduledValues"](0),
            0 === s)
                return void (e["value"] = t);
            const o = this._audioContext["currentTime"];
            switch (s += o,
            i) {
            case 0:
                ...
                break;
            case 1:
                e["setValueAtTime"](e["value"], o),
                e["linearRampToValueAtTime"](t, s);
                break;
            case 2:
                e["setValueAtTime"](e["value"], o),
                e["exponentialRampToValueAtTime"](t, s)
            }
        }
        GetState() {
            return {
    ...
    self.C3AudioFilterFX = class extends e {
        constructor(e, t, i, s, o, a, n) {
            ...
            this._filterNode["type"] = t,
            this._filterNode["frequency"]["value"] = i,
            this._filterNode["detune"]["value"] = s,
            this._filterNode["Q"]["value"] = o,
            this._filterNode["gain"]["vlaue"] = a,
        ...
        SetParam(e, t, i, s) {
            switch (e) {
            case 0:
                t = Math.max(Math.min(t / 100, 1), 0),
                this._params[5] = t,
                this.SetAudioParam(this._wetNode["gain"], t, i, s),
                this.SetAudioParam(this._dryNode["gain"], 1 - t, i, s);
                break;
            case 1:
                this._params[1] = t,
                this.SetAudioParam(this._filterNode["frequency"], t, i, s);
                break;
            case 2:
                this._params[2] = t,
                this.SetAudioParam(this._filterNode["detune"], t, i, s);
                break;
            case 3:
                this._params[3] = t,
                this.SetAudioParam(this._filterNode["Q"], t, i, s);
                break;
            case 4:
                this._params[4] = t,
                this.SetAudioParam(this._filterNode["gain"], t, i, s)
            }
        }
    }
    ,
    self.C3AudioDelayFX = class extends e {
        constructor(e, t, i, s) {
            ...
            this._inputNode = this.CreateGain(),
            this._wetNode = this.CreateGain(),
            this._wetNode["gain"]["value"] = s,
            this._dryNode = this.CreateGain(),
            this._dryNode["gain"]["value"] = 1 - s,
            ...
            this._delayGainNode["gain"]["value"] = i,
        ...
        SetParam(e, t, i, s) {
            const o = self.AudioDOMHandler.DbToLinear;
            switch (e) {
            case 0:
                t = Math.max(Math.min(t / 100, 1), 0),
                this._params[2] = t,
            ...
            case 4:
                this._params[1] = o(t),
                this.SetAudioParam(this._delayGainNode["gain"], o(t), i, s);
                break;
            case 5:
                this._params[0] = t,
                this.SetAudioParam(this._delayNode["delayTime"], t, i, s)
            }
        }
    }
    ,
    self.C3AudioConvolveFX = class extends e {
        constructor(e, t, i, s) {
            ...
            this._convolveNode["normalize"] = i,
    ...
    self.C3AudioFlangerFX = class extends e {
        constructor(e, t, i, s, o, a) {
            super(e),
            this._type = "flanger",
            this._params = [t, i, s, o, a],
            this._inputNode = this.CreateGain(),
            this._dryNode = this.CreateGain(),
            this._dryNode["gain"]["value"] = 1 - a / 2,
            this._wetNode = this.CreateGain(),
            this._wetNode["gain"]["value"] = a / 2,
            this._feedbackNode = this.CreateGain(),
            this._feedbackNode["gain"]["value"] = o,
            this._delayNode = this._audioContext["createDelay"](t + i),
            this._delayNode["delayTime"]["value"] = t,
            this._oscNode = this._audioContext["createOscillator"](),
            this._oscNode["frequency"]["value"] = s,
        ...
        SetParam(e, t, i, s) {
            ...
            case 0:
                t = Math.max(Math.min(t / 100, 1), 0),
                this._params[4] = t,
                this.SetAudioParam(this._wetNode["gain"], t / 2, i, s),
                this.SetAudioParam(this._dryNode["gain"], 1 - t / 2, i, s);
                break;
            case 6:
                this._params[1] = t / 1e3,
                this.SetAudioParam(this._oscGainNode["gain"], t / 1e3, i, s);
                break;
            case 7:
                this._params[2] = t,
                this.SetAudioParam(this._oscNode["frequency"], t, i, s);
                break;
            case 8:
                this._params[3] = t / 100,
                this.SetAudioParam(this._feedbackNode["gain"], t / 100, i, s)
            }
        }
    }
    ,
    self.C3AudioPhaserFX = class extends e {
        constructor(e, t, i, s, o, a, n) {
            ...
            this._filterNode["frequency"]["value"] = t,
            this._filterNode["detune"]["value"] = i,
            this._filterNode["Q"]["value"] = s,
            this._oscNode = this._audioContext["createOscillator"](),
            this._oscNode["frequency"]["value"] = a,
            this._oscGainNode = this.CreateGain(),
            this._oscGainNode["gain"]["value"] = o,
            this._inputNode["connect"](this._filterNode),
            this._inputNode["connect"](this._dryNode),
            this._filterNode["connect"](this._wetNode),
            this._oscNode["connect"](this._oscGainNode),
        ...
        SetParam(e, t, i, s) {
            ...
            case 6:
                this._params[3] = t,
                this.SetAudioParam(this._oscGainNode["gain"], t, i, s);
                break;
            case 7:
                this._params[4] = t,
                this.SetAudioParam(this._oscNode["frequency"], t, i, s)
    ...
    self.C3AudioGainFX = class extends e {
        ...
        SetParam(e, t, i, s) {
            ...
            if (4 === e)
                this._params[0] = o(t),
                this.SetAudioParam(this._node["gain"], o(t), i, s)
        }
    }
    ,
    self.C3AudioStereoPanFX = class extends e {
        constructor(e, t) {
            super(e),
            this._type = "stereopan",
            this._params = [t],
            this._node = this._audioContext["createStereoPanner"](),
            this._node["pan"]["value"] = t
        }
        Release() {
            this._node["disconnect"](),
            super.Release()
        ...
        SetParam(e, t, i, s) {
            if (t = Math.min(Math.max(t / 100, -1), 1),
            9 === e)
                this._params[0] = t,
                this.SetAudioParam(this._node["pan"], t, i, s)
        }
    }
    ,
    self.C3AudioTremoloFX = class extends e {
        constructor(e, t, i) {
            ...
            this._params = [t, i],
            this._node = this.CreateGain(),
            this._node["gain"]["value"] = 1 - i / 2,
            this._oscNode = this._audioContext["createOscillator"](),
            this._oscNode["frequency"]["value"] = t,
            this._oscGainNode = this.CreateGain(),
            this._oscGainNode["gain"]["value"] = i / 2,
            this._oscNode["connect"](this._oscGainNode),
        ...
        SetParam(e, t, i, s) {
            ...
            case 0:
                t = Math.max(Math.min(t / 100, 1), 0),
                this._params[1] = t,
                this.SetAudioParam(this._node["gain"], 1 - t / 2, i, s),
                this.SetAudioParam(this._oscGainNode["gain"], t / 2, i, s);
                break;
            case 7:
                this._params[0] = t,
                this.SetAudioParam(this._oscNode["frequency"], t, i, s)
    ...
    self.C3AudioRingModFX = class extends e {
        constructor(e, t, i) {
            ...
            this._inputNode = this.CreateGain(),
            this._wetNode = this.CreateGain(),
            this._wetNode["gain"]["value"] = i,
            this._dryNode = this.CreateGain(),
            this._dryNode["gain"]["value"] = 1 - i,
    ...
    self.C3AudioDistortionFX = class extends e {
        constructor(e, t, i, s, o, a) {
            ...
            this._GenerateColortouchCurve(t, i),
        ...
        _Shape(e, t, i) {
            const s = 1.05 * i * t - t
              , o = e < 0 ? -1 : 1
              , a = e < 0 ? -e : e;
            let n = a < t ? a : t + s * self.AudioDOMHandler.e4(a - t, 1 / s);
            return n *= o,
            n
        }
        ConnectTo(e) {
    ...
    self.C3AudioCompressorFX = class extends e {
        constructor(e, t, i, s, o, a) {
            ...
            this._node["threshold"]["value"] = t,
            this._node["knee"]["value"] = i,
            this._node["ratio"]["value"] = s,
            this._node["attack"]["value"] = o,
            this._node["release"]["value"] = a
        }
        Release() {
            this._node["disconnect"](),
            super.Release()
    ...
    self.C3AudioAnalyserFX = class extends e {
        constructor(e, t, i) {
            ...
            this._node["smoothingTimeConstant"] = i,
...
"use strict";
{
    const e = "mouse"
      , t = class extends self.DOMHandler {
        constructor(t) {
            super(t, e),
            this.AddRuntimeMessageHandlers([["cursor", e => this._OnChangeCursorStyle(e)], ["request-pointer-lock", e => this._OnRequestPointerLock(e)], ["release-pointer-lock", () => this._OnReleasePointerLock()]]),
        ...
        _OnRequestPointerLock(e) {
            this._iRuntime.GetMainCanvas().requestPointerLock(e)
        }
...
"use strict";
{
    ...
    const n = class extends self.DOMElementHandler {
        constructor(t) {
            super(t, e)
        }
        CreateElement(e, n) {
            ...
            d.addEventListener("pointerrawupdate", t),
            d.addEventListener("pointerup", t),
            d.addEventListener("mousedown", t),
            d.addEventListener("mouseup", t),
            d.addEventListener("keydown", t),
            d.addEventListener("keyup", t),
            d.addEventListener("contextmenu", e => e.preventDefault()),
            s.addEventListener("click", () => this._PostToRuntimeElementMaybeSync("click", e, {
                "isChecked": s.checked
            })),
            n["id"] && (s.id = n["id"]),
            n["className"] && (s.className = n["className"]),
            a && n["usesAutoFontSize"] && s.classList.add("c3-auto-size-checkbox"),
            this.UpdateState(d, n),
            d
        }
        _GetInputElem(e) {
        ...
        UpdateState(e, t) {
            const n = this._GetInputElem(e);
            n.checked = t["isChecked"],
            n.disabled = !t["isEnabled"],
            e.title = t["title"],
            e === n ? n.value = t["text"] : e.lastChild.textContent = t["text"]
        }
    }
    ;
    self.RuntimeInterface.AddDOMHandlerClass(n)
}
...
{
    const r = "keyboard"
      , o = class extends self.DOMHandler {
        constructor(o) {
            super(o, r),
            this._isKeyboardLockSupported = !(!navigator["keyboard"] || !navigator["keyboard"]["lock"]),
            this.AddRuntimeMessageHandlers([["init", () => this._OnInit()], ["lock-keyboard", r => this._OnLockKeyboard(r)], ["unlock-keyboard", () => this._OnUnlockKeyboard()]])
        }
        _OnInit() {
            return {
        ...
        async _OnLockKeyboard(r) {
            const o = r["keysArr"];
            try {
                return 0 === o.length ? await navigator["keyboard"]["lock"]() : await navigator["keyboard"]["lock"](o),
                {
                    "isOk": !0
                }
            } catch (r) {
                return console.error("Error locking keyboard:", r),
                {
                    "isOk": !1
                }
            }
        ...
        _OnUnlockKeyboard() {
            try {
                ...
                console.error("Error unlocking keyboard:", r)
            }
        }
    }
    ;
    self.RuntimeInterface.AddDOMHandlerClass(o)
}
