( () => {
    var n = {
        ...
        612: () => {
            ...
            window.initPokiBridge = function(e) {
                n = e,
                window.unityGame ? window.pokiReady || window.pokiAdBlock ? window.pokiReady ? window.unityGame.SendMessage(e, "ready") : window.pokiAdBlock && window.unityGame.SendMessage(e, "adblock") : window.pokiBridge = e : setTimeout(( () => {
                    window.initPokiBridge(e)
                }
                ), 100)
            }
            ,
        ...
        989: n => {
            !function(e) {
                ...
                if (d)
                    ...
                    var v = k.prototype = x.prototype = Object.create(p);
                    S.prototype = v.constructor = k,
                    k.constructor = S,
                    ...
                    ,
                    d.mark = function(n) {
                        return Object.setPrototypeOf ? Object.setPrototypeOf(n, k) : (n.__proto__ = k,
    ...
    ( () => {
        ...
        const b = "image";
        let x, S, k = 0, E = !1;
        const L = p.screenshotsVersion ? `?v${p.screenshotsVersion}` : ""
          , I = window.location.pathname.substring(0, window.location.pathname.lastIndexOf("/"))
          , $ = window.location.hostname.endsWith("game-cdn.poki.com") || window.location.hostname.endsWith(".poki-gdn.com") ? `/cdn-cgi/image/f=auto,quality=78${I}/` : "";
        async function T() {
            ...
            if (e.getAttribute("fullImageLoaded"))
                ...
                window.slideShowTimeoutID = window.setTimeout(C, n);
            else {
                const i = Date.now()
                  , o = await N(`${$}screenshots/${t + 1}.jpg${L}`);
                e.querySelector("img").src = o.src,
                e.setAttribute("fullImageLoaded", !0),
                clearTimeout(window.slideShowMoveTransitionID),
                clearTimeout(window.slideShowTimeoutID);
                const r = Date.now() - i;
                r > n ? C() : window.slideShowTimeoutID = window.setTimeout(C, n - r)
            }
        }
        function C() {
            if (E)
            ...
            function(n) {
                k = n | 0;
                const e = k > 0 ? k - 1 : p.numScreenshots - 1
                  , t = k < p.numScreenshots - 1 ? k + 1 : 0;
        ...
        function z() {
            ...
            M.style.width = `${n}px`,
            M.style.height = `${e}px`,
            t > p.maxRatio ? M.style.width = e * p.maxRatio + "px" : t < p.minRatio && (M.style.height = n / p.minRatio + "px");
            const i = M.getBoundingClientRect();
            M.style.marginLeft = -.5 * i.width + "px",
            M.style.marginTop = -.5 * i.height + "px"
        }
        function _(n) {
            const e = 100 * n;
            D.style.width = `${e}%`,
            ...
            n >= 1 && "done" !== R.className && (R.className = "done",
        ...
        function H() {
            ...
            G++,
            G >= n.length && (G = 0),
            P = setTimeout(H, p.commentChangeTime)) : j.innerHTML = ""
        }
        function q() {
            const n = document.createElement("script")
              , e = "Build";
            n.src = `${e}/${p.metadata.loader_filename}`,
            p.dataUrl = `${e}/${p.metadata.data_filename}`,
            p.frameworkUrl = `${e}/${p.metadata.framework_filename}`,
            p.codeUrl = `${e}/${p.metadata.code_filename}`,
            p.streamingAssetsUrl = "StreamingAssets",
            n.addEventListener("load", ( () => {
                const n = document.querySelector("#game");
                createUnityInstance(n, p, _).then((n => {
                    window.unityGame = n,
        ...
        window.onload = () => {
            ...
            R = document.getElementById("progress-container"),
            D = document.getElementById("progress-fill"),
            ...
            try {
                (async function() {
                    ...
                    w.className = "active";
                    try {
                        n = await N(`${$}screenshots/1-small.jpg${L}`)
                    } catch (e) {
                        n = await N(`${$}screenshots/1.jpg${L}`)
                    }
                    const e = B();
                    ...
                    const t = await N(`${$}screenshots/1.jpg${L}`);
                    e.querySelector("img").src = t.src;
                    const i = n => {
                        const e = B()
                          , t = new Image;
                        t.src = `${$}screenshots/${n + 1}-small.jpg${L}`,
                        e.appendChild(t),
                        e.setAttribute("data-idx", n),
        ...
        V.src = J;
        const Z = K("slideshow-top-container")
          , nn = K("game-title");
        nn.innerText = p.title;
        const en = K("progress-spinner");
        en.innerHTML = '<div class="bounce0"></div><div class="bounce1"></div><div class="bounce2">',
        en.setAttribute("class", "spinner");
        const tn = K("progress-container")
          , on = K("progress-bar")
          , rn = K("progress-fill");
        rn.style.width = "0%";
        const an = K("progress-amount");
        an.innerText = "0%";
        const sn = K("progress-comment");
        sn.innerText = "Loading";
        const ln = K("slideshow-images")
          , dn = K("slideshow-nav")
          , cn = K("game-container")
          , hn = document.createElement("canvas");
        hn.setAttribute("id", "game"),
        W.appendChild(X),
        X.appendChild(Y),
        X.appendChild(ln),
        X.appendChild(dn),
